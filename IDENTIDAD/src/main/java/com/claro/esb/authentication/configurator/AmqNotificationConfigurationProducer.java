...
public class AmqNotificationConfigurationProducer {
	...
	@Autowired
	private AmqAudit producer;

	@Bean("mail-connection-factory")
	public ConnectionFactory connectionFactory() {
		String brokerURL;
		brokerURL = producer.getBorkerUrl();

		ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory();
		connectionFactory.setBrokerURL(brokerURL);
		connectionFactory.setUserName(producer.getUser());
		connectionFactory.setPassword(producer.getPassword());
	...
	public RestTemplate restTemplate() {
	    ...
	    Proxy proxy = new Proxy(Type.HTTP, new InetSocketAddress("proxy.claro.co", 8080));
